...
public class CitycodeDBHelper {
     ...
     * @return
     */
    public Cursor fuzzySearch(String cityName) {
        String selection = DefineSQL.CityCodeDB.COLUMN_CITY + " like '%" + cityName + "%' or " + DefineSQL.CityCodeDB.COLUMN_COUNTY
                + " like '%" + cityName + "%' or " + DefineSQL.CityCodeDB.COLUMN_PROVINCE + " like '%" + cityName + "%'";
        return mDatabase.rawQuery("select * from " + DefineSQL.CityCodeDB.TABLE_NAME + " where " + selection ,null);
    }
